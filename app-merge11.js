...
function exportOcrExcel() {
  const lines = getOcrLines();
  if (!lines.length) {
    toast('暂无可导出文字', 'error');
    return;
  }
  const rows = lines.map((text, index) => ({ 行号: index + 1, 识别文字: text }));
  const wb = rowsToWorkbook(rows, ['行号', '识别文字'], '识别文字');
...
function getPreparedOcrSource(item) {
  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      const scale = enhance ? Math.min(3, Math.max(1.4, 1800 / Math.max(baseWidth, baseHeight))) : 1;
...
function enhanceOcrCanvas(ctx, canvas) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    value = value > 188 ? 255 : value < 118 ? 0 : value;
