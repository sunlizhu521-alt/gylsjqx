const tools = [
  { id: 'merge', icon: '▦', title: '报表合并', status: '可用-陈燕提的需求', desc: '多个文件按映射列合并，保留明细行。' },
  { id: 'compare', icon: '⌁', title: '多列对比', status: '可用-陈燕提的需求', desc: '选择多列数据，按行找最大值、最小值。' },
  { id: 'ocr', icon: '文', title: '图片转文字', status: '调试中', desc: '上传图片后识别文字，并导出 TXT、Excel、Word。' },
...
function renderOcrPanel() {
  return `
    ...
    <label class="field control-card"><span>识别增强</span><select id="ocrEnhance"><option value="on" selected>开启：放大、灰度、增强对比</option><option value="off">关闭：使用原图</option></select></label>
    ...
    <div class="button-row ocr-actions">
      ...
      <button class="secondary-button" id="exportOcrExcel" type="button">转 Excel</button>
...
function bindOcrControls() {
  ...
  $('exportOcrExcel')?.addEventListener('click', exportOcrExcel);
...
function renderOcrTextPreview() {
  const lines = getOcrLines().map((text, index) => ({ 行号: index + 1, 识别文字: text }));
  renderTable(['行号', '识别文字'], lines);
...
async function runOcr() {
  try {
    ...
    app.ocrText = result.data.text.trim();
    app.ocrWords = normalizeOcrWords(result.data.words || []);
    if (output) output.value = app.ocrText;
    renderOcrTextPreview();
...
function exportOcrExcel() {
  const table = buildOcrExcelTable();
  if (!table.length) {
    toast('暂无可导出文字', 'error');
    return;
  }
  const ws = XLSX.utils.aoa_to_sheet(table);
  ws['!cols'] = getOcrColumnWidths(table);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, '识别文字');
  XLSX.writeFile(wb, getOcrBaseName() + '_识别文字.xlsx');
}
...
function buildOcrExcelTable() {
  if (!app.ocrWords.length) {
    return getOcrLines().map(line => [line]);
  }

  const rows = groupOcrWordsIntoRows(app.ocrWords);
  const rowCells = rows.map(row => splitOcrRowIntoCells(row)).filter(row => row.length);
  if (!rowCells.length) return getOcrLines().map(line => [line]);

  const columns = clusterOcrColumns(rowCells.flat());
  return rowCells.map(cells => {
    const output = Array(columns.length).fill('');
    cells.forEach(cell => {
      const index = findNearestOcrColumn(columns, cell.x0);
      output[index] = output[index] ? `${output[index]} ${cell.text}` : cell.text;
    });
...
function groupOcrWordsIntoRows(words) {
  ...
  const medianHeight = median(sorted.map(word => word.y1 - word.y0)) || 16;
...
function splitOcrRowIntoCells(words) {
  const gaps = [];
  for (let index = 1; index < words.length; index += 1) {
    gaps.push(words[index].x0 - words[index - 1].x1);
  }
  const medianGap = median(gaps.filter(gap => gap > 0)) || 12;
  const medianWidth = median(words.map(word => word.x1 - word.x0)) || 24;
  const splitGap = Math.max(18, medianGap * 2.4, medianWidth * 0.65);
  const cells = [];
  let current = [];
  words.forEach((word, index) => {
    if (index > 0 && word.x0 - words[index - 1].x1 > splitGap) {
      cells.push(makeOcrCell(current));
      current = [];
    }
    current.push(word);
  });
  if (current.length) cells.push(makeOcrCell(current));
  return cells;
}

function makeOcrCell(words) {
  return {
...
function clusterOcrColumns(cells) {
  const sorted = [...cells].sort((a, b) => a.x0 - b.x0);
  const medianWidth = median(sorted.map(cell => cell.x1 - cell.x0)) || 80;
  const tolerance = Math.max(24, medianWidth * 0.45);
  const columns = [];
  sorted.forEach(cell => {
    let column = columns.find(item => Math.abs(item.x - cell.x0) <= tolerance);
    if (!column) {
      column = { x: cell.x0, count: 0 };
      columns.push(column);
    }
    column.x = (column.x * column.count + cell.x0) / (column.count + 1);
    column.count += 1;
  });
  return columns.sort((a, b) => a.x - b.x).map(column => column.x);
}

function findNearestOcrColumn(columns, x) {
  let bestIndex = 0;
...
function getOcrColumnWidths(table) {
  const maxColumns = Math.max(...table.map(row => row.length));
  return Array.from({ length: maxColumns }, (_, index) => {
    const width = Math.max(10, Math.min(42, ...table.map(row => String(row[index] || '').length + 4)));
    return { wch: width };
  });
}
...
function getPreparedOcrSource(item) {
  const enhance = $('ocrEnhance')?.value !== 'off';
  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      const scale = enhance ? Math.min(4, Math.max(1.6, 2400 / Math.max(baseWidth, baseHeight))) : 1;
      ...
      if (enhance) enhanceOcrCanvas(ctx, canvas);
      canvas.toBlob(blob => {
...
function enhanceOcrCanvas(ctx, canvas) {
  const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  const data = imageData.data;
  for (let i = 0; i < data.length; i += 4) {
    const gray = data[i] * 0.299 + data[i + 1] * 0.587 + data[i + 2] * 0.114;
    let value = (gray - 128) * 1.7 + 128;
