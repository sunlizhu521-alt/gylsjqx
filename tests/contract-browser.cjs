...
(async () => {
  ...
  try {
    ...
    for (const [index, title] of ['PRINT PREVIEW PAGE 1', 'PRINT PREVIEW PAGE 2'].entries()) {
      const sheet = previewPdf.addPage([595.28, 841.89]);
      sheet.drawText(title, { x: 92, y: 730, size: 24, color: rgb(.08, .16, .3) });
      sheet.drawText(`Source PDF page ${index + 1} - layout is rendered from the uploaded PDF.`, { x: 92, y: 690, size: 12 });
    }
    ...
    assert.equal(await page.locator('#mappingStage').isHidden(), true);
    await page.locator('#previewFile').setInputFiles({ name: '错误预览.pdf', mimeType: 'application/pdf', buffer: Buffer.from('not-a-pdf') });
    assert.match(await page.locator('#contractStatus').innerText(), /有效的PDF/);
    assert.equal(await page.locator('#mappingStage').isHidden(), true);
    await page.locator('#previewFile').setInputFiles({ name: '虚构合同模板-打印预览.pdf', mimeType: 'application/pdf', buffer: Buffer.from(previewPdfBytes) });
    assert.equal(await page.locator('#mappingStage').isHidden(), true);
    await page.locator('#previewMatchConfirm').check();
    await page.waitForFunction(() => !document.querySelector('#mappingStage').hidden);
    assert.equal(await page.locator('#showPrintPreview').getAttribute('aria-pressed'), 'true');
    assert.equal(await page.locator('#previewPagination').getAttribute('data-total-pages'), '2');
    await page.waitForFunction(() => document.querySelector('.pdf-preview-canvas')?.dataset.rendered === 'true');
    assert.equal(await page.locator('.pdf-preview-canvas').getAttribute('data-page'), '1');
    assert.equal(await page.locator('#previewPrevious').isDisabled(), true);
    await page.locator('#previewNext').click();
    assert.equal(await page.locator('#previewPageLabel').innerText(), '第 2 / 2 页');
    ...
    assert.ok(Number(await page.locator('.pdf-preview-canvas').getAttribute('width')) > 0);
    await page.locator('#mappingStage').screenshot({ path: path.join(out, 'contract-pdf-page2.png') });
    await page.locator('#previewPrevious').click();
    await page.locator('#showMappingStructure').click();
    assert.equal(await page.locator('#showMappingStructure').getAttribute('aria-pressed'), 'true');
    assert.match(await page.locator('.mapping-structure-note').innerText(), /不代表合同打印排版/);
    ...
    await page.locator('#previewMatchConfirm').check();
    await page.waitForFunction(() => !document.querySelector('#mappingStage').hidden);
    assert.match(await page.locator('#contractStatus').innerText(), /模板中的图形、数据透视表、结构化表将在生成文件中保留/);
    await page.locator('#showMappingStructure').click();
    ...
    await page.locator('#showPrintPreview').click();
    await page.waitForFunction(() => document.querySelector('.pdf-preview-canvas')?.dataset.rendered === 'true');
    await page.locator('#mappingStage').scrollIntoViewIfNeeded(); await page.waitForTimeout(150);
    ...
    console.log(JSON.stringify({ status: 'PASS', output: out, screenshots: ['contract-pdf-page2.png', 'contract-mapping-structure.png', 'contract-desktop.png', 'contract-word-result.png', 'contract-complex-upload.png', 'contract-excel.png', 'contract-mobile.png'] }));
