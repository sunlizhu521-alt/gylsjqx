...
(async () => {
  ...
  try {
    ...
    assert.ok(Number(await page.locator('#previewPagination').getAttribute('data-total-pages')) > 1);
    const a4Box = await page.locator('.a4-page').boundingBox();
    assert.ok(Math.abs(a4Box.width / a4Box.height - 210 / 297) < 0.01);
    assert.equal(await page.locator('#previewPrevious').isDisabled(), true);
    await page.locator('#previewNext').click();
    assert.match(await page.locator('#previewPageLabel').innerText(), /第 2 \/ /);
    assert.match(await page.locator('#templatePreview').innerText(), /附加条款/);
    await page.locator('#mappingStage').screenshot({ path: path.join(out, 'contract-a4-page2.png') });
    await page.locator('#previewPrevious').click();
    assert.match(await page.locator('#previewPageLabel').innerText(), /第 1 \/ /);
    ...
    await page.locator('#mappingStage').screenshot({ path: path.join(out, 'contract-excel-page2.png') });
    await page.locator('#previewPrevious').click();
    ...
    console.log(JSON.stringify({ status: 'PASS', output: out, screenshots: ['contract-a4-page2.png', 'contract-excel-page2.png', 'contract-desktop.png', 'contract-word-result.png', 'contract-complex-upload.png', 'contract-excel.png', 'contract-mobile.png'] }));
