...
映射方案按模板 SHA-256 指纹保存在当前浏览器，也可以导入、导出 JSON；不缓存订单、合同模板和生成结果。Word 模板暂不支持文本框、图片、域、修订和嵌套表格；Excel 模板暂不支持宏、外部链接、保护、图形、透视表、结构化表，以及需要在明细扩展时自动改写的下方公式。GitHub Pages 无法运行 LibreOffice，因此页面只提供内容预览，不声称是 Office 实际 PDF；PDF 请下载合同后在 Word 或 Excel 中另存。
