...
访问 `contract.html`，上传 `.xlsx` / `.xls` 订单明细和 `.docx` / `.xlsx` 合同模板。文件只在浏览器本地处理，程序不会上传、覆盖或改名原模板。模板以 A4 纸张比例分页预览，可用上一页、下一页逐页检查并点击纸张中的位置配置订单字段。单值字段存在多个不同值时必须明确选择取第一条、合并、手工输入或求和；表格或 Excel 行可设置为明细模板行，按订单逐行扩展。确认文件名和映射后生成新的 DOCX 或 XLSX。
